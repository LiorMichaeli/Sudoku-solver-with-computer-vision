"""
    ...
    Functions:
        get_sudoku_board_as_array_from_img(arg_img_path: str, arg_initialized_numbers_in_sodoku_by_order: List)
...
def get_sudoku_board_as_array_from_img(arg_img_path: str, arg_initialized_numbers_in_sodoku_by_order: List[int]) -> List[List[int]]:
    ...
    This is a function that gets img path and the numbers that were initialized in sodoku by order
         from left to right and top to bottom, and returns 2D array of numbers that represents the sudoku board.
    :param arg_img_path:
    :param arg_initialized_numbers_in_sodoku_by_order:
...
def __get_sudoku_board_as_gray_scale_img(arg_img_after_pre_processing: np.ndarray) -> np.ndarray:
    try:
        top_left_corner, top_right_corner, bottom_right_corner, bottom_left_corner = __get_corners_of_sudoku_board(arg_img_after_pre_processing)
    except Exception as e:
        raise e
...
def __get_sudoku_board_cells_in_sorted_order(arg_sudoku_board_gray_scale_img: np.ndarray) -> List[np.ndarray]:
    sudoku_board_cells = []
    cell_size = get_cell_size_in_sudoku_board(arg_sudoku_board_gray_scale_img)
    for row_index in range(constants.NUMBER_OF_ROWS_IN_SUDOKU_BOARD):
        for col_index in range(constants.NUMBER_OF_COLS_IN_SUDOKU_BOARD):
            ...
            digit_img = arg_sudoku_board_gray_scale_img[
...
def __convert_cells_imgs_to_numbers(arg_sudoku_board_gray_scale_img: np.ndarray, arg_cells_imgs_list_in_sorted_order: np.ndarray,
                                    arg_initialized_numbers_in_sodoku_by_order: List[int]) -> List[List[int]]:
    cell_size = get_cell_size_in_sudoku_board(arg_sudoku_board_gray_scale_img)
    ...
    for index, cell in enumerate(arg_cells_imgs_list_in_sorted_order):
        # Checks if the cell contains number
        if cell.sum() >= threshold:
            sudoku_board_row.append(arg_initialized_numbers_in_sodoku_by_order[iterator_for_initialized_numbers_in_sodoku])
...
def __get_corners_of_sudoku_board(arg_img_after_pre_processing: np.ndarray) -> Tuple[List[int], List[int], List[int], List[int]]:
...
def __get_corners_of_largest_square_in_img(arg_contours: List[np.ndarray]) -> Union[np.ndarray, None]:
